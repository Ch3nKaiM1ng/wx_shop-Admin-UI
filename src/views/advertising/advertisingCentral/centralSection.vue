...
<script>
import { imgList, delImg } from '@/api/navigation'
export default {
  ...
  methods: {
    ...
    imgDatalist() {
      ...
      imgList({ setData: 1 }).then(response => {
    ...
    // 去添加图片
    goAddImg() {
      this.$router.push({ path: '/advertising/addpicture' })
    },
    // 去编辑图片
    motifyImg(row) {
      this.$router.push({ path: '/advertising/addpicture', query: { id: row }})
    },
    // 删除图片
    deleteImg(row) {
      var vm = this
